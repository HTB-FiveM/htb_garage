Config.Garages = {
    Garage_Postcode_584 = {
		Type = 'car',
		-- Impound = {
		-- 	Pos = {x=279.77, y=-345.87, z=44.92},
		-- 	Marker = { w= 1.5, h= 1.0,r = 204, g = 204, b = 0},
		-- 	HelpPrompt = _U('open_car_garage')
		-- },
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x=288.15, y= -343.09, z=44.92},
			Heading = 160.0,
			Marker = { w= 1.5, h= 1.0,r=0,g=255,b=0},
			NumSpawns = 3,
			HelpPrompt = _U('spawn_car')
		},
		DeletePoint = {
			Pos = {x=272.62, y=-337.21, z=44.92},
			Marker = { w= 1.5, h= 1.0,r=255,g=0,b=0},
			HelpPrompt = _U('store_car')
		}
	},
	Garage_postcode_381 = {
		Type = 'car',
		-- Impound = {
		-- 	Pos = {x=-333.44, y=-692.44, z=32.94},
		-- 	Marker = { w= 1.5, h= 1.0,r = 204, g = 204, b = 0},
		-- 	HelpPrompt = _U('open_car_garage')
		-- },
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x=-324.71, y= -682.81, z=32.95},
			Heading = 360.0,
			Marker = { w= 1.5, h= 1.0,r=0,g=255,b=0},
			NumSpawns = 4,
			HelpPrompt = _U('spawn_car')
		},
		DeletePoint = {
			Pos = {x=-352.56, y=-691.57, z=32.55},
			Marker = { w= 1.5, h= 1.0,r=255,g=0,b=0},
			HelpPrompt = _U('store_car')
		}
	},
	Garage_VinewoodBowl = {
		Type = 'car',
		-- Impound = {
		-- 	Pos ={x = 664.54406738281,y = 629.0244140625,z = 128.91111755371 },
		-- 	Marker = { w= 1.5, h= 1.0,r = 204, g = 204, b = 0},
		-- 	HelpPrompt = _U('open_car_garage')
		-- },
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x = 657.646,y = 630.719,z = 128.911 },
			Heading = 340.0,
			Marker = { w= 1.5, h= 1.0,r=0,g=255,b=0},
			NumSpawns = 7,
			HelpPrompt = _U('spawn_car')
		},
		DeletePoint = {
			Pos = {x = 650.57611083984,y = 625.05859375,z = 128.9111328125 },
			Marker = { w= 1.5, h= 1.0,r=255,g=0,b=0},
			HelpPrompt = _U('store_car')
		}
	},
	Garage_Centre2_6259 = {
		Type = 'car',
		-- Impound = {
		-- 	Pos = {x = -1523.191,y = -451.017,z = 35.596},
		-- 	Marker = { w= 1.5, h= 1.0,r = 204, g = 204, b = 0},
		-- 	HelpPrompt = _U('open_car_garage')
		-- },
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x = -1517.1,y = -446.1,z = 35.44},
			Heading = 195.12,
			Marker = { w= 1.5, h= 1.0,r=0,g=255,b=0},
			NumSpawns = 2,
			HelpPrompt = _U('spawn_car')
		},
		DeletePoint = {
			Pos = {x = -1528.022,y = -443.280,z = 35.45},
			Marker = { w= 1.5, h= 1.0,r=255,g=0,b=0},
			HelpPrompt = _U('store_car')
		}
	},
	Garage_Paleto = {
		Type = 'car',
		-- Impound = {
		-- 	Pos = {x=105.359, y=6613.586, z=32.3973},
		-- 	Marker = { w= 1.5, h= 1.0,r = 204, g = 204, b = 0},
		-- 	HelpPrompt = _U('open_car_garage')
		-- },
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x=150.64, y=6578.44, z= 31.86},
			Heading = 224.1,
			Marker = { w= 1.5, h= 1.0,r=0,g=255,b=0},
			NumSpawns = 6,
			HelpPrompt = _U('spawn_car')
		},
		DeletePoint = {
			Pos = {x=140.98, y=6605.65, z=31.85},
			Marker = { w= 1.5, h= 1.0,r=255,g=0,b=0},
			HelpPrompt = _U('store_car')
		}
	},
	Garage_SandyShore = {
		Type = 'car',
		-- Impound = {
		-- 	Pos = {x=1694.571, y=3610.924, z=35.319},
		-- 	Marker = { w= 1.5, h= 1.0,r = 204, g = 204, b = 0},
		-- 	HelpPrompt = _U('open_car_garage')
		-- },
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x=1716.62, y= 3593.21, z= 35.33},
			Heading = 207.71,
			Marker = { w= 1.5, h= 1.0,r=0,g=255,b=0},
			NumSpawns = 2,
			HelpPrompt = _U('spawn_car')
		},
		DeletePoint = {
			Pos = {x = 1695.156,y = 3601.061,z = 35.54},
			Marker = { w= 1.5, h= 1.0,r=255,g=0,b=0},
			HelpPrompt = _U('store_car')
		}
	},
	Garage_Ocean1 = {
		Type = 'car',
		-- Impound = {
		-- 	Pos = {x = -3140.323,y = 1124.463,z = 20.706},
		-- 	Marker = { w= 1.5, h= 1.0,r = 204, g = 204, b = 0},
		-- 	HelpPrompt = _U('open_car_garage')
		-- },
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x = -1140.63, y = 2671.77, z = 18.09},
			Heading = 310.5,
			Marker = { w= 1.5, h= 1.0,r=0,g=255,b=0},
			NumSpawns = 3,
			HelpPrompt = _U('spawn_car')
		},
		DeletePoint = {
			Pos = {x = -1155.01, y = 2666.6,z = 18.09},
			Marker = { w= 1.5, h= 1.0,r=255,g=0,b=0},
			HelpPrompt = _U('store_car')
		}
	},
	Garage_Ocean2 = {
		Type = 'car',
		-- Impound = {
		-- 	Pos = {x = -2982.561,y = 327.506,z = 14.935},
		-- 	Marker = { w= 1.5, h= 1.0,r = 204, g = 204, b = 0},
		-- 	HelpPrompt = _U('open_car_garage')
		-- },
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x = -2977.238,y = 337.777,z = 14.77},
			Heading = 160.0,
			Marker = { w= 1.5, h= 1.0,r=0,g=255,b=0},
			NumSpawns = 5,
			HelpPrompt = _U('spawn_car')
		},
		DeletePoint = {
			Pos = {x = -2978.1, y = 351.85,z = 14.79},
			Marker = { w= 1.5, h= 1.0,r=255,g=0,b=0},
			HelpPrompt = _U('store_car')
		}
	},
	Garage_Sud = {
		Type = 'car',
		-- Impound = {
		-- 	Pos = {x = 443.6696,y = -1969.4372,z = 24.401},
		-- 	Marker = { w= 1.5, h= 1.0,r = 204, g = 204, b = 0},
		-- 	HelpPrompt = _U('open_car_garage')
		-- },
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x = 450.54,y = -1972.29,z = 22.56},
			Heading = 182.0,
			Marker = { w= 1.5, h= 1.0,r=0,g=255,b=0},
			HelpPrompt = _U('spawn_car')
		},
		DeletePoint = {
			Pos = {x = 457.91,y = -1971.343,z = 22.99},
			Marker = { w= 1.5, h= 1.0,r=255,g=0,b=0},
			HelpPrompt = _U('store_car')
		}
	},
	Garage_Lake = {
		Type = 'car',
		-- Impound = {
		-- 	Pos = {x = -73.165504455566,y = 908.08734130859,z = 235.62712097168 },
		-- 	Marker = { w= 1.0, h= 1.0,r = 204, g = 204, b = 0},
		-- 	HelpPrompt = _U('open_car_garage')
		-- },
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x = -75.19,y = 898.01,z = 235.56 },
			Heading = 122.58,
			Marker = { w= 1.0, h= 1.0,r=0,g=255,b=0},
			NumSpawns = 1,
			HelpPrompt = _U('spawn_car')
		},
		DeletePoint = {
			Pos = {x = -66.987632751465,y = 891.65881347656,z = 235.55270385742 },
			Marker = { w= 1.0, h= 1.0,r=255,g=0,b=0},
			HelpPrompt = _U('store_car')
		}
	},
	Garage_Occaz = {
		Type = 'car',
		-- Impound = {
		-- 	Pos = {x = 1136.3771972656,y = 2666.6303710938,z = 38.013275146484 },
		-- 	Marker = { w= 1.5, h= 1.0,r = 204, g = 204, b = 0},
		-- 	HelpPrompt = _U('open_car_garage')
		-- },
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x = 1120.9813232422,y = 2668.8684082031,z = 38.048095703125 },
			Heading = 264.6,
			Marker = { w= 1.5, h= 1.0,r=0,g=255,b=0},
			NumSpawns = 4,
			HelpPrompt = _U('spawn_car')
		},
		DeletePoint = {
			Pos = {x = 1121.9963378906,y = 2660.1179199219,z = 37.996875762939 },
			Marker = { w= 1.5, h= 1.0,r=255,g=0,b=0},
			HelpPrompt = _U('store_car')
		}
	},
	Garage_Groove = {
		Type = 'car',
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x = -73.27, y = -2000.03, z = 18.02 },
			Heading = 263.43,
			Marker = { w= 1.5, h= 1.0,r=0,g=255,b=0},
			NumSpawns = 5,
			HelpPrompt = _U('spawn_car')
		},
		DeletePoint = {
			Pos = {x = -78.54, y = -2011.25, z = 18.02 },
			Marker = { w= 1.5, h= 1.0,r=255,g=0,b=0},
			HelpPrompt = _U('store_car')
		}
	},
	Garage_venise = {
		Type = 'car',
		-- Impound = {
		-- 	Pos = {x = -1081.974609375,y = -1254.1881103516,z = 5.4244246482849 },
		-- 	Marker = { w= 1.5, h= 1.0,r = 204, g = 204, b = 0},
		-- 	HelpPrompt = _U('open_car_garage')
		-- },
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x = -1075.16, y = -1258.04, z = 5.28 },
			Heading = 144.887,
			Marker = { w= 1.5, h= 1.0,r=0,g=255,b=0},
			HelpPrompt = _U('spawn_car')
		},
		DeletePoint = {
			Pos = {x = -1097.2971191406,y = -1253.8725585938,z = 5.2441983222961 },
			Marker = { w= 1.5, h= 1.0,r=255,g=0,b=0},
			HelpPrompt = _U('store_car')
		}
	},
	Garage_tequila = {
		Type = 'car',
		-- Impound = {
		-- 	Pos = {x = -575.66357421875,y = 318.41366577148,z = 84.614906311035 },
		-- 	Marker = { w= 1.5, h= 1.0,r = 204, g = 204, b = 0},
		-- 	HelpPrompt = _U('open_car_garage')
		-- },
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x = -566.15, y = 328.0, z = 84.43 },
			Heading = 261.5,
			NumSpawns = 7,
			Marker = { w= 1.5, h= 1.0,r=0,g=255,b=0},
			HelpPrompt = _U('spawn_car')
		},
		DeletePoint = {
			Pos = {x = -568.44, y = 335.77, z = 84.48 },
			Marker = { w= 1.5, h= 1.0,r=255,g=0,b=0},
			HelpPrompt = _U('store_car')
		}
	},
	Garage_banque = {
		Type = 'car',
		-- Impound = {
		-- 	Pos = {x = 363.48370361328,y = 296.83682250977,z = 103.50011444092 },
		-- 	Marker = { w= 1.5, h= 1.0,r = 204, g = 204, b = 0},
		-- 	HelpPrompt = _U('open_car_garage')
		-- },
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x = 378.00622558594,y = 288.13024902344,z = 103.1661529541 },
			Heading = 62.22,
			Marker = { w= 1.5, h= 1.0,r=0,g=255,b=0},
			NumSpawns = 2,
			HelpPrompt = _U('spawn_car')
		},
		DeletePoint = {
			Pos = {x = 364.76132202148,y = 285.18911743164,z = 103.37410736084 },
			Marker = { w= 1.5, h= 1.0,r=255,g=0,b=0},
			HelpPrompt = _U('store_car')
		}
	},
	Garage_mirrorpark = {
		Type = 'car',
		-- Impound = {
		-- 	Pos = {x = 1033.9229736328,y = -767.10662841797,z = 58.003326416016 },
		-- 	Marker = { w= 1.5, h= 1.0,r = 204, g = 204, b = 0},
		-- 	HelpPrompt = _U('open_car_garage')
		-- },
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x = 1040.26, y = -769.18, z = 58.02 },
			Heading = 359.92,
			Marker = { w= 1.5, h= 1.0,r=0,g=255,b=0},
			NumSpawns = 3,
			HelpPrompt = _U('spawn_car')
		},
		DeletePoint = {
			Pos = {x = 1022.7816772461,y = -763.78955078125,z = 57.961227416992 },
			Marker = { w= 1.5, h= 1.0,r=255,g=0,b=0},
			HelpPrompt = _U('store_car')
		}
	},
    Garage_Lsc = {
		Type = 'car',
		-- Impound = {
		-- 	Pos = {x = -467.15,y = -274.87,z = 35.91 },
		-- 	Marker = { w= 1.0, h= 1.0,r = 204, g = 204, b = 0},
		-- 	HelpPrompt = _U('open_car_garage')
		-- },
		SpawnPoint = {
            Name = _U('garage_name'),
			Pos = {x = -462.22, y = -269.34, z = 35.78 },
			Heading = 25.0,
			Marker = { w= 1.0, h= 1.0,r=0,g=255,b=0},
			NumSpawns = 1,
			HelpPrompt = _U('spawn_car')
	    },
	    DeletePoint = {
			 Pos = {x = -460.62,y = -273.04,z = 35.79 },
			 Marker = { w= 1.0, h= 1.0,r=255,g=0,b=0},
			 HelpPrompt = _U('store_car')
        }
	},
    Garage_Sunrise = {
		Type = 'car',
        -- Impound = {
        --     Pos = {x = -777.31,y = -225.85,z = 37.18 },
        --     Marker = { w= 1.5, h= 1.0,r = 204, g = 204, b = 0},
        --     HelpPrompt = _U('open_car_garage')
        -- },
        SpawnPoint = {
            Name = _U('garage_name'),
            Pos = {x = -772.6,y = -237.1,z = 37.15 },
            Heading = 214.87,
            Marker = { w= 1.5, h= 1.0,r=0,g=255,b=0},
            HelpPrompt = _U('spawn_car')
        },
        DeletePoint = {
            Pos = {x = -769.13,y = -235.53,z = 37.18 },
            Marker = { w= 1.5, h= 1.0,r=255,g=0,b=0},
            HelpPrompt = _U('store_car')
		}
	}

}