...
function HtbGarageUtil.RequestModel(modelHash, cb)
    modelHash = (type(modelHash) == 'number' and modelHash or `modelHash`) -- Using backticks instead of GetHashKey
    
    if not HasModelLoaded(modelHash) and IsModelInCdimage(modelHash) then
        HtbGarageUtil.RequestModel(modelHash)
...
function SpawnVehicle(vehicle, coords, heading, cb, networked)
    local model = (type(vehicle) == 'number' and vehicle or `vehicle`) -- Using backticks instead of GetHashKey
...
function SetBaseVehicleProperties(vehicle, props)
