...
function HandleVehicleSpecifics(vehicleEntity, vehicleType, deleteZone)
    if vehicleType == 'boat' and deleteZone.DryPoint then
        ...
        TriggerServerEvent('htb_garage:TeleportAllInVehicleToDock',
            playerServerIds,
            {
                            ...
                            callback = function()
                                ...
                                if IsControlJustReleased(0, 38) and IsInputDisabled(0) and GetLastInputMethod(2) then
                                    if not IsPedInAnyVehicle(PlayerPedId()) then
                                        ...
                                        if zoneName == "DeletePoint" then StoreVehicle(vehicleType, zone) end
...
function PayForRetrieve(vehicle)
    ...
    Citizen.Trace("function PayForRetrieve: Unable to find account 'money' for player " .. playerData.identifier .. "\n")
    return false
end
...
function DetermineSpawnPosition(vehicleType, spawnPoint, heading, numSpawns)
    ...
    for currentTry = 0, totalSpawns - 1, 1 do
        ...
        local pos = vector2(spawnPos.x, spawnPos.y) 
            - (hVect * firstSpawnOffset) 
            - (hVect * offset)
        
        if Config.Debug then
            table.insert(debugPostList, { x = pos.x, y = pos.y, z = spawnPos.z - .98 })
        end
...
AddEventHandler('htb_garage:ResultsForVehicleSpawn', function(vehicle, garageName)
    ...
    local spawnPoint = DetermineSpawnPosition(vehicle.type, garage.SpawnPoint, garage.SpawnPoint.Heading, garage.SpawnPoint.NumSpawns)
